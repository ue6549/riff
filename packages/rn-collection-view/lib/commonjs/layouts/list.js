...
class ListLayout {
  ...
  _listEngine = nativeMod.listLayout;
  ...
  prepare(context) {
    ...
    listDebugLog(`[RNCVX-LIST] Sending to C++ this._listEngine.computeSections:`, sectionParams.map(s => ({
    ...
    listMvcTrace(`prepare: fingerprintChanged=${fingerprintChanged} sections=${sectionParams.length} totalItems=${sectionParams.reduce((n, s) => n + s.itemCount, 0)}`);
    if (fingerprintChanged) {
      ...
      if (H) this._cache.stashMeasuredSizes();else this._cache.stashHeights();
    ...
    if (RNCV_MVC_TRACE_LAYOUT && __DEV__) {
      sectionParams.forEach((sec, sIdx) => {
        const itemHeights = sec.itemHeights;
  ...
  shouldInvalidate(oldBounds, newBounds) {
    ...
    return this.horizontal ? Math.abs(oldBounds.height - newBounds.height) > 0.5 : Math.abs(oldBounds.width - newBounds.width) > 0.5;
  }
  invalidationScope(_oldBounds, _newBounds) {
    return {
      type: 'full'
    };
  }
  invalidateFrom(key, context) {
